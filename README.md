...
> ⚠️ **このブランチ `demo-app-with-eval` は、デモ版 (`demo-app`) に「絵文字入力の精度評価・モデル比較」を加えたものです。**
> 空中フリックの入力メカニクス（**折り曲げ式の日本語・英語入力**、**手の回転による言語切替**、**入力テストページ**、**UI の日本語/英語切替**）を
> 実際に手で試して調整できる試作に加え、**手描き → 絵文字検索の精度 (top-1/5/10・MRR) と速度を、CLIP モデル (ViT-B/32 / B/16 / L/14) をページ内で切り替えて比較する評価モード**を持つ。
> 運指・しきい値は**画面から可変**にしてあり、安定版ではありません。
> デモ内容は [`docs/fold_input_demo.md`](docs/fold_input_demo.md)、**精度評価・モデル比較（データ配置・起動）** は [`packages/web/README.md`](packages/web/README.md) にまとめています。
...
| モダリティ | パッケージ | 状態 | 概要 |
|---|---|---|---|
| 絵文字入力 | `emoji-search` | **実装済み (v0)** | OpenMoji を CLIP で埋め込み、テキスト / 手書き画像 / カメラで検索。手描き入力の**精度評価・モデル比較 (ViT-B/32 / B/16 / L/14) モード**あり |
...
- **精度評価のモデル比較 (ViT-B/32 / B/16 / L/14) を使う場合**は、モデル別の index を Drive から取得し `MIDAIR_MODELS` で起動する。データ配置・URL・起動手順は [`packages/web/README.md`](packages/web/README.md) を参照。
