...
export function applyEmojiLayout(isEmoji) {
  ...
  if (btn) {
    ...
    btn.textContent = evalMode ? t("eval.modeBtnBack") : t("eval.modeBtn");
  }
}
...
export function toggleEvalMode() {
  ...
  if (evalMode) setStatus(t("eval.selectSettings"));
}
...
export async function startEval() {
  ...
  setStatus(t("eval.fetching"));
  let data;
  try { data = await (await fetch(`/api/eval/targets?n=${n}&group=${encodeURIComponent(group)}`)).json(); }
  catch (e) { setStatus(t("eval.fetchErr", { e })); return; }
  if (!data.targets || !data.targets.length) { setStatus(t("eval.noTargets")); return; }
...
export function stopEval() {
  setActive(false);
  setStatus(t("eval.stopped"));
  renderEvalLog();
}

function showTarget() {
  const tg = state.targets[state.idx];
  const p = $("evalProgress"); if (p) p.textContent = t("eval.progress", { i: state.idx + 1, n: state.targets.length });
  const img = $("evalTargetImg"); if (img) img.src = tg.image_url;
  const lab = $("evalTargetLabel"); if (lab) lab.textContent = `${tg.emoji} ${tg.label}`;
  setStatus(t("eval.drawPrompt", { label: tg.label }));
...
function startCountdown() {
  ...
  const render = () => {
    ...
    el.textContent = t("eval.remaining", { s: r.toFixed(1) });
...
export async function evalSubmit(forced = false) {
  ...
  if (blank && !forced) { setStatus(t("eval.drawFirst")); return; }  // 空クリックはカウントダウン継続
  ...
  if (!blank) {                                       // 未描画(時間切れ)は検索せず圏外(rank=null)で記録
    ...
    catch (e) { setStatus(t("eval.searchErr", { e })); busy = false; startCountdown(); return; }
  ...
  state.idx++;
  busy = false;
  if (state.idx >= state.targets.length) { setActive(false); setStatus(t("eval.done")); }
  else { clearPad(); showTarget(); }
  renderEvalLog();
}

export function evalClearLog() {
  if (!confirm(t("eval.clearConfirm"))) return;
...
function rankBadge(rank) {
  ...
  return `<span class="badge bmiss">${t("eval.miss")}</span>`;
}
...
const limitLabel = (lim) => (lim ? `${lim}s` : t("eval.unlimited"));
...
export function renderEvalLog() {
  ...
  let cmp = `<table class="eval-table"><thead><tr>
      <th>${t("eval.thModel")}</th><th>${t("eval.thLimit")}</th><th>${t("eval.thTrials")}</th><th>Top-1</th><th>Top-3</th><th>Top-5</th><th>Top-10</th><th>MRR</th>
      <th>${t("eval.thAvgInput")}</th><th>${t("eval.thAvgSearch")}</th></tr></thead><tbody>`;
  ...
  // --- 各試行の詳細 ---
  let rows = "";
  for (const tr of log.slice().reverse()) {
    const resImgs = tr.results.map((r) => {
      const hit = r.id === tr.target.hexcode ? " hit" : "";
      return `<img class="res${hit}" src="${r.image_url}" title="${r.label} (${r.score})" loading="lazy" />`;
    }).join("");
    const ml = (tr.model && tr.model.label) || "?";
    const cond = limitLabel(tr.limitSec || 0)
      + (tr.timedOut ? t("eval.timedOut") : "") + (tr.blank ? t("eval.blankTrial") : "");
    const dsec = tr.drawMs != null ? `${(tr.drawMs / 1000).toFixed(1)}s` : "-";
    const sms = tr.searchMs != null ? `${Math.round(tr.searchMs)}ms` : "-";
    rows += `<tr>
      <td class="tgt"><img src="${tr.target.image_url}" loading="lazy" /><div>${tr.target.emoji} ${tr.target.label}</div></td>
      <td><img class="draw" src="${tr.drawing}" /></td>
      <td>${rankBadge(tr.rank)}<div class="trial-meta">${ml} · ${cond}<br>${t("eval.inputSearch", { d: dsec, s: sms })}</div></td>
  ...
  box.innerHTML = `
    <div class="eval-summary">
      <h2>${t("eval.summaryTitle")} <button onclick="evalClearLog()" style="float:right">${t("eval.clearLog")}</button></h2>
      ${cmp}
    </div>
    <table class="eval-table" style="margin-top:20px">
      <thead><tr><th>${t("eval.thPrompt")}</th><th>${t("eval.thDrawing")}</th><th>${t("eval.thResult")}</th><th>${t("eval.thTop10")}</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
}

// 言語切替時に評価UIを現在言語で再描画する (core.js の applyLang から呼ばれる)。
// 静的DOM(data-i18n)は applyStaticI18n が処理するので、ここは動的生成部分だけを更新する。
export function applyEvalI18n() {
  renderEvalLog();                                   // 集計/詳細テーブルを現在言語で再構築
  if (state.active && state.targets[state.idx]) {    // セッション進行中: お題ラベルも追従
    const tg = state.targets[state.idx];
    const p = $("evalProgress"); if (p) p.textContent = t("eval.progress", { i: state.idx + 1, n: state.targets.length });
    setStatus(t("eval.drawPrompt", { label: tg.label }));
  } else if (evalMode) {                             // 設定待機中: 案内ステータス
    setStatus(t("eval.selectSettings"));
  }
}
